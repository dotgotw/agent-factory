...
**交辦訊息裡帶足夠的狀態**:PR 編號、分支名、當下 `main` 的 SHA、對方上一封講過的
具體事實。理由不是禮貌 —— peer name 每一輪重生,而後綴只有兩個字元;**過期的名字
今天會乾淨失敗,但那只需要一次巧合就會變成「悄悄送到另一個活著的 session」**。
帶著狀態的訊息讓收錯的人一眼看得出「這不是給我的」,把一個會變沉默的失敗壓回吵鬧。

### 同一個理由的另一半:你自己明天也不能靠這條 session

交辦訊息要帶狀態,是因為**收訊息的人**不能靠你的 session。而那條 session 對
**你自己**也一樣不可靠 —— 差別只在它消失得慢一點。

所以:**一個任務一條 session,任務換了就換。** 不是因為 context 會滿,是因為
context 是稅率不是餘額 —— 上一個任務留下的那一段,從任務結束的那一刻起就只剩
稅,沒有用途(價目與判準在 `AGENTS.md`「session 是拋棄式的」)。

到了任務邊界卻關不掉,那不是「再撐一下」的理由,是**狀態還沒落地的信號**:
把決策寫進 `contract/decisions/`、跨 scope 的開 CR、下一個人會再踩的留坑註解。
寫完就關得掉了 —— 這正是上面「訊息不在 repo 裡」那句話的反面用法。
