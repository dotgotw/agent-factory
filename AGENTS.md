...
**先問:下一個人會不會在同樣的地方再踩一次?** 不會 → 寫進 PR 描述,結束。

會 → 家在「他再踩的時候眼睛會看到的地方」:改那段程式的人看註解,寫測試的人看測試,
按合併鍵的人看 `README.md`,開工第一分鐘的人看該角色的 `AGENTS.md`。
**踩到的地方不在你的 scope 裡 → 開 CR**,不要只傳訊息:訊息不在 repo 裡,
對方的 session 一壓縮就沒了。

第一個實例就地留註解並指名下一個會踩的人(`// 坑(下一個踩的人:qa):…`),
